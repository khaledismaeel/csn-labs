...
This section offers the background necessary for understanding this document. First, we present the application under consideration, T4, in _Introduction_. Next, in _Scope_, we set the boundaries for what is expected in this document. Finally, we conclude with _Contents_ where we draw the roadmap for the rest of the document.
...
| RN-2 | The system shall minimize storage usage by compressing and rotating all the readings. |
...
- **Device adapters**. These are microservices that pull raw data from a family of sensors and format them into a stream of discrete samples, the schema of which depends on the device and metric type.
- **Aggregator**. This component collects the sample streams generated by the device adapters and aggregates their output into a device-independent format. It consists of the following.
